<?php
        ...
        //}
            ...
            while($row=mysqli_fetch_array($pConsulta)){
                ...
                if(mysqli_num_rows($pConsulta2)<1){
                    ...
                    $Consulta2 = "SELECT realizado FROM disperciones WHERE idMov = $row[0] AND realizado = 0";
                    $pConsulta2 = consulta_tb($mysqli, $Consulta2);
                    $Consulta3 = "SELECT realizado FROM movimientoFinal WHERE idMov = $row[0] AND realizado = 0";
                    $pConsulta3 = consulta_tb($mysqli, $Consulta3);
    ...
    function tabla_mov_incompletos_nominas_facturar($mes, $anio) {
        ...
        $Consulta = "SELECT a.id, a.idMov, a.fecha, b.origen, (SELECT b.nombre FROM empresas b WHERE b.id = a.empresa), a.monto, a.folio, a.pdf FROM movimientoNominas a, movimiento b WHERE b.id = a.idMov AND b.activo = 1 AND a.realizado = 1 AND a.fecha BETWEEN '$anio-$mes-1' AND '$anio-$mes-31' ORDER BY a.id ASC";
