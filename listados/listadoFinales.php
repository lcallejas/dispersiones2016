<?php
    ...
    if($_SESSION){
        ...
        if(!isset($_SESSION['mesListadoFinales'])){
            $_SESSION['mesListadoFinales'] = 0;
            $_SESSION['anioListadoFinales'] = 0;
        }

        if (isset($_POST['meses'])){
            $_SESSION['mesListadoFinales'] = $_POST['meses'];
            $_SESSION['anioListadoFinales'] = $_POST['anio'];
        }
                ...
                <form method="POST" name="formulario" id="formulario" enctype="multipart/form-data">
                    ...
                    <select name="meses" id="meses">
                        <?php
                            for ($i=0; $i<=12; $i++) {
                                echo "<option value='$i'";
                                if($i == $_SESSION['mesListadoFinales']){
                                    echo "selected";
                                }
                                echo ">$meses[$i]</option>";
                            }
                    ...
                    <select name="anio" id="anio">
                        ...
                        <?php
                            for ($i=2015; $i<=2050; $i++) {
                                echo "<option value='$i'";
                                if($i == $_SESSION['anioListadoFinales']){
                                    echo "selected";
                                }
                                echo ">$i</option>";
                            }
                    ...
                    <input type="button" value="Aceptar"  onClick="visualizar('ListadoFinales')">
                    <h2>Finales:</h2>
                    <?php
                        echo "<table id='tabla_incompletos' border=1>";
                            tabla_listado_mov_finales($_SESSION['mesListadoFinales'], $_SESSION['anioListadoFinales']);
                        echo"</table>";
                    ?>
                    <br><br>
                </form>
